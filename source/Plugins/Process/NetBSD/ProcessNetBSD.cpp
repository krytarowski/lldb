                               ...
                               const FileSpec *crash_file_path)
{
    ...
    if (crash_file_path == NULL)
        process_sp.reset(new ProcessNetBSD (target_sp, listener, GetNetBSDSignals()));
    return process_sp;
}
...
ProcessNetBSD::ProcessNetBSD(lldb::TargetSP target_sp, Listener &listener, UnixSignalsSP &unix_signals_sp)
...
#else
        // Special case when F_DUPFD_CLOEXEC does not exist (Debian kNetBSD)
...
ProcessNetBSD::UpdateThreadList(ThreadList &old_thread_list, ThreadList &new_thread_list)
{
    ...
    if (!thread_sp) {
        thread_sp.reset(CreateNewNetBSDThread(*this, GetID()));
        has_updated = true;
    }

    if (log && log->GetMask().Test(POSIX_LOG_VERBOSE))
        log->Printf ("ProcessNetBSD::%s() updated pid = %" PRIi64, __FUNCTION__, GetID());
    new_thread_list.AddThread(thread_sp);
